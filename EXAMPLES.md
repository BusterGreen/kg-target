...
```lua
exports['kg-target']:AddBoxZone("MissionRowDutyClipboard", vector3(441.7989, -982.0529, 30.67834), 0.45, 0.35, {
...
exports['kg-target']:AddTargetModel(Config.Peds, {
	options = {
		{
...
exports['kg-target']:AddTargetEntity(mule, {
    options = {
        {
            type = "client",
...
## Passing Item Data
In this example, we define the model of the coffee machines you see around the map, and allow players to purchase a coffee. You'll have to provide your own logic for the purchase, but this is how you would handle it with kg-target, and how you would pass data through to an event for future use.
...
```lua
exports['kg-target']:AddTargetModel(`prop_vend_coffe_01`, {
    options = {
        {
            type = "client",
            event = "coffee:buy",
...
RegisterNetEvent('coffee:buy',function(data)
    ...
    KGCore.Functions.Notify("You purchased a " .. data.label .. " for $" .. data.price .. ". Enjoy!", 'success')
end)
```
...
AddEventHandler('plantpotato',function()
	...
	exports['kg-target']:AddEntityZone("potato-growing-"..plant, plant, {
